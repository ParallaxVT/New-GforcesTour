function Add-GforcesHtmlFiles {
    ...
    $template_content |
    ForEach-Object { ($_).replace('SERVERNAME',$configXML.tour.url) } |
    ForEach-Object { ($_).replace('SCENENAME',$tour) } |
    Out-File -Encoding utf8 $dir\$tour\index.html
    # devel.xml
    $template_content |
    ForEach-Object { ($_).replace('SERVERNAME','..') } |
    ForEach-Object { ($_).replace('SCENENAME',$tour) } |
    ForEach-Object { ($_).replace('tour.xml','devel.xml') } |
...
function Add-GforcesDevelXml {
    ...
    ForEach-Object { Add-Content $develFile ('    <include url="%CURRENTXML%/content/' + $_.BaseName + '.xml" />') }
    Add-Content $develFile '    <!-- Include -->'
    $includefolder = Get-ChildItem "$dir\shared\includev2\"  |
    ForEach-Object { Add-Content $develFile ('    <include url="%SWFPATH%/includev2/' + $_.BaseName + '/index.xml" />') }
    Add-Content $develFile '    <!-- Scenes -->'
    $scenesfolder = Get-ChildItem "$dir\$tour\files\scenes\*.xml"  |
    ForEach-Object { Add-Content $develFile ('    <include url="%CURRENTXML%/scenes/' + $_.BaseName + '.xml" />') }
...
function Add-ToTourXml ($selectedFolder) {
    foreach ($xmlFile in $selectedFolder) {
        ...
        Where-Object { $_ -notmatch "<krpano" -and $_ -notmatch "</krpano" -and $_.trim() -ne "" } |
...
function Add-GforcesTourXml {
    ...
    if (Test-Path "$dir\$tour\files\scenes\scene_1_a.xml") {
        ...
        $sceneContent |
        ForEach-Object { ($_).replace('<scene name="scene_1_a">',$newScenename) } |
...
function Add-GfocesBrandsIndex {
    ...
    ForEach-Object {
        if ($_ -match 'ADDPAGETITLE' ) {
            '        <title>NetDirector Car Interiors</title>'
        }
        elseif ($_ -match 'ADDTITLE' ) {
            '                    <h1>NetDirector Car Interiors</h1>'
        }
        elseif ($_ -match 'ADDBREADCRUMB' ) {
            ''
        }
        elseif ($_ -match 'ADDCONTENT' ) {
            '                    <ul>'
    ...
    ForEach-Object {($_).replace('NEWPATH','..')} |
    ForEach-Object {($_).replace('HOMEPATH','.')} |
    ForEach-Object {($_).replace('./brands/index.html','./index.html')} |
    ForEach-Object {($_).replace('.brands {','.brands {display:none;')} |
    ...
    foreach ($item in $tourArray) {
        ...
        foreach ($country in $tour | Where-Object { $_.id -like $countryId } ) {
            ...
            if (!($customtour)) {
                ...
                ForEach-Object {
                    if ($_ -match 'ADDPAGETITLE' ) {
                        '            <title> NetDirector - ' + $country.name + '</title>'
                    }
                    elseif ($_ -match 'ADDTITLE' ) {
                        '            <h1>' + $country.name + '</h1>'
                    }
                    ...
                    elseif ($_ -match 'ADDCONTENT' ) {
                        foreach ($brand in $country.brand) {
                            '                    <h2><a id="' + $brand.id + '" href="./' + $($brand.id) + '/index.html">' + $brand.name + '</a></h2>'
                            '                    <ul>'
                ...
                ForEach-Object {($_).replace('NEWPATH','../..')} |
                ForEach-Object {($_).replace('HOMEPATH','..')} |
                ForEach-Object {($_).replace('All Brands','Grid View')} |
                ForEach-Object {($_).replace('./brands/index.html','./grid_more.html')} |
                Set-Content "$countryFolder\index.html"
                Write-Verbose "   > $($country.id)\index.html file"
                # devel.html
                (Get-Content "$countryFolder\index.html") |
                ForEach-Object { ($_).replace('</style>','.home-content{background:palegoldenrod;}</style>') } |
                ForEach-Object { ($_).replace('index.html','devel.html') } |
            ...
            if ($country.id -like 'gb' ) {
                ...
                ForEach-Object {($_).replace('gb_','ie_')} |
                ForEach-Object {($_).replace('Great Britain','Ireland')} |
            ...
            if ($customtour) {
                ...
                ForEach-Object {
                    ...
                    elseif ($_ -match 'ADDCONTENT' ) {
                        foreach ($brand in $country.brand) {
                            '            <h2><a name="' + $brand.id + '" href="./' + $($brand.id) + '/index.html">' + $brand.name + '</a></h2>'
                            '            <ul>'
                            foreach ($model in $brand.model) {
                                foreach ($car in $model.car) {
                                    'INSERT' + $brand.id
                ...
                foreach ($brand in $country.brand ) {
                    ...
                    ForEach-Object { ($_).replace("$ccorig","$ccdest") } |
                    Set-Content "$countryFolder\index.html"
...
function Add-GforcesRecent {
    ...
    ForEach-Object {
        if ($_ -match 'ADDPAGETITLE' ) {
            '    <title>Recent Car Interiors</title>'
        }
        elseif ($_ -match 'ADDTITLE' ) {
            '                    <h1>Recent Car Interiors</h1>'
        }
        elseif ($_ -match 'ADDBREADCRUMB' ) {
            ''
        }
    ...
    ForEach-Object {($_).replace('NEWPATH','..')} |
    ForEach-Object {($_).replace('HOMEPATH','.')} |
    ForEach-Object {($_).replace('id="main-list"','id="recent"')} |
    ForEach-Object {($_).replace('./brands/index.html','./index.html')} |
    ForEach-Object {($_).replace('SERVERNAME',$configXML.tour.url) } |
        ...
        foreach ($country in $tour | Where-Object { $_.id -like $countryId } ) {
            foreach ($brand in $country.brand | Where-Object { $_.id -like $brandId } ) {
                ...
                if (!($customtour)) {
                    ...
                    ForEach-Object {
                        if ($_ -match 'ADDPAGETITLE' ) {
                            '    <title>NetDirector - ' + $country.name + ' - ' + $brand.name + '</title>'
                        }
                        elseif ($_ -match 'ADDTITLE' ) {
                            '                    <h1>' + $brand.name + '</h1>'
                        }
                        elseif ($_ -match 'ADDBREADCRUMB' ) {
                                ...
                                '                        <ol class="breadcrumb">'
                                '                            <li class="breadcrumb-item"><a href="../../index.html">Home</a></li>'
                                '                            <li class="breadcrumb-item"><a href="../index.html">' + $country.name + '</a></li>'
                    ...
                    ForEach-Object {($_).replace('NEWPATH','../../..')} |
                    ForEach-Object {($_).replace('HOMEPATH','../..')} |
                    ForEach-Object {($_).replace('All Brands','Dark Interface')} |
                    ForEach-Object {($_).replace('./brands/index.html','./brand.html')} |
                    ...
                    if ($country.id -like 'gb' ) {
                        ...
                        ForEach-Object {($_).replace('/gb/','/ie/')} |
                        ForEach-Object {($_).replace('Great Britain','Ireland')} |
                ...
                # Do the following block only when the function is called for more countries
                    if ($customtour) {
                        ...
                        foreach ($item in $allModels) {
                            $html_content = Get-Content $item.FullName
                            $html_content |
                            ForEach-Object { ($_).replace("$($country.id)","$($country.dest)") } |
                            ForEach-Object { ($_).replace('Great Britain',"$($country.name)") } |
                        ...
                        foreach ($item2 in $allCars) {
                            ...
                            $html_content = Get-Content $itemPath
                            $html_content |
                            ForEach-Object { ($_).replace("$($country.id)","$($country.dest)") } |
                            ForEach-Object { ($_).replace('Great Britain',"$($country.name)") } |
        ...
        foreach ($country in $tour | Where-Object { $_.id -like $countryId } ) {
            foreach ($brand in $country.brand | Where-Object { $_.id -like $brandId } ) {
                foreach ($model in $brand.model) {
                    ...
                    ForEach-Object {
                        if ($_ -match 'ADDPAGETITLE' ) {
                            '    <title>NetDirctor - ' + $country.name + ' - ' + $brand.name + ' - ' + $model.name +  '</title>'
                        }
                        elseif ($_ -match 'ADDTITLE' ) {
                        ...
                        elseif ($_ -match 'ADDBREADCRUMB' ) {
                                ...
                                '                            <li class="breadcrumb-item"><a href="../../../index.html">Home</a></li>'
                                '                            <li class="breadcrumb-item"><a href="../../index.html">' + $country.name + '</a></li>'
                                '                            <li class="breadcrumb-item"><a href="../index.html">' + $brand.name + '</a></li>'
...
function Add-GforcesGridBrands {
    ...
    $tempfile = "$dir\.src\html\index.temp"
        ...
        foreach ($country in $tour | Where-Object { $_.id -like $countryId } ) {
            ...
            foreach ($brand in $country.brand) {
                $brand_name = $brand.id
                Add-Content $tempfile ('                <div class="col-lg-3 col-md-4 col-6"><a href="./' + $($brand.id) + '/brand.html" class="d-inline-block mb-5"><img class="img-fluid img-thumbnail" src="../../shared/html_brands/img/logos/' + $($brand.id) + '.jpg" alt="' + $($brand.name) + '"></a></div>')
            }
            ...
            $template_content | ForEach-Object {
                if ($_ -match 'ADDPAGETITLE' ) {
                    '            <title>NetDirctor Assets - Interiors</title>'
                }
                elseif ($_ -match 'ADDTITLE' ) {
                    '            <h1>NetDirector Assets - Interiors</h1>'
                }
                elseif ($_ -match 'ADDCONTENT' ) {
                    $brands_content
                }
            ...
            ForEach-Object { ($_).replace('brand.html','more_brands.html') } |
    ...
    foreach ($item in $tourArray) {
        ...
        foreach ($country in $tour | Where-Object { $_.id -like $countryId } ) {
            foreach ($brand in $country.brand | Where-Object { $_.id -like $brandId } ) {
                ...
                $template_content |
                ForEach-Object { ($_).replace('SERVERNAME',$configXML.tour.url) } |
                ForEach-Object { ($_).replace('BRANDNAME',$brandname) } |
                ForEach-Object { ($_).replace('SCENENAME',$first_car) } |
                ...
                ForEach-Object { ($_).replace('SERVERNAME',$configXML.tour.url) } |
                ForEach-Object { ($_).replace('BRANDNAME',$brandname) } |
                ForEach-Object { ($_).replace('SCENENAME',($first_car + ',null,more')) } |
                ...
                ForEach-Object { ($_).replace('SCENENAME',$first_car) } |
                ForEach-Object { ($_).replace('BRANDNAME',$brandname) } |
                ForEach-Object { ($_).replace('brand.xml','devel_brand.xml') } |
                ...
                $template_content |
                ForEach-Object { ($_).replace('SERVERNAME','../../..') } |
                ForEach-Object { ($_).replace('BRANDNAME',$brand_name) } |
                ForEach-Object { ($_).replace('SCENENAME',($first_car + ',null,more')) } |
                ForEach-Object { ($_).replace('../../../brands',"../../../brands/$($country.id)") } |
                ForEach-Object { ($_).replace('brand.xml','devel_brand.xml') } |
    ...
    foreach ($item in $tourArray) {
        ...
        foreach ($country in $tour | Where-Object { $_.id -like $countryId } ) {
            foreach ($brand in $country.brand | Where-Object { $_.id -like $brandId } ) {
                ...
                Add-Content $develFile '    <!-- Include -->'
                $includefolder = Get-ChildItem "$dir\shared\include_brand\"  |
                ForEach-Object { Add-Content $develFile ('    <include url="%SWFPATH%/include_brand/' + $_.BaseName + '/index.xml" />') }
                Add-Content $develFile '    <!-- Scenes -->'
                foreach ($model in $brand.model) {
                    foreach ($car in $model.car) {
                        if ($ignoreArray -notcontains $car.id) {
...
function Rename-CarsWithWrongName {
    ...
    $renameThisCars = $tourIDArray + $renameArray.id | Select-Object -Unique
    ...
    # Run 2 ForEach-Object loops to get the 'renameTo' attribute
    foreach ($item in $renameThisCars) {
        foreach ($renameCar in $renameArray | Where-Object {$_.id -like $item})  {
            $carID = $($renameCar.id)
            ...
            $index_content |
            ForEach-Object { ($_).replace($CarID,$carRenameTo ) } |
...
function Duplicate-GforcesCars {
    ...
    if ($ignoreArray -notcontains $tourArray) {
        ...
        foreach ($item in $tourIDArray) {
            $countryId = ($item -split "_")[0]
            $brandId = ($item -split "_")[1]
            foreach ($country_duplicate in $duplicateItems | Where-Object { $_.id -like $countryId } ) {
                foreach ($brand_duplicate in $country_duplicate.brand | Where-Object { $_.id -like $brandId -or $_.id -like 'all' } ) {
                    ...
                    if ($ignoreArray -notcontains $car_origin) {
                        ...
                        $template_content |
                        ForEach-Object { ($_).replace('SERVERNAME',$configXML.tour.url) } |
                        ForEach-Object { ($_).replace('SCENENAME',$item) } |
                    ...
                    if(Test-Path $dir\$car_origin\files\scenes\scene.xml) {
                        ...
                        $origin_scene |
                        ForEach-Object { ($_).replace('name="' + $countrycode + '_', 'name="' + $dest + '_') } |
